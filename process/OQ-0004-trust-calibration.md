...
- The content and format of Nexus Strategic Briefings
- Whether the framework tracks and surfaces agent accuracy metrics over time
- The NEXUS CHECK and NEXUS MERGE experience design

## Options Being Considered

**Option A — Track record metrics:**
The framework maintains per-agent accuracy metrics: how often does the Coder's output pass verification on first attempt? How often does the Planner's effort estimate match actual effort? These metrics are surfaced in Strategic Briefings to give the human data-driven trust signals.
...
Each agent outputs a self-assessed confidence score (per DEC-0007). The framework calibrates these scores against actual outcomes over time and presents calibrated confidence in briefings.
...
Start with tight human oversight (human reviews every task output, not just gates). As track record builds, progressively relax oversight to the standard two-gate model. The framework suggests autonomy level adjustments based on track record.
...
Ship v1 with the two-gate model and no trust calibration features. Gather user feedback on whether trust is a real problem in practice.
...
- The Nexus Strategic Briefing format may need trust-related data fields
