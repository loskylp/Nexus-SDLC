...
15. **Clean up your own mess, not the project's.** When your task removes a function, import, or variable, remove what becomes unused *because of your change*. Do not delete pre-existing dead code, unrelated unused imports, or stale comments that were already there when you arrived — they may be load-bearing in ways your task does not reveal, and silent cleanup spreads the diff beyond what the Verifier can trace to the acceptance criteria. If you notice pre-existing dead code, mention it in the handoff note; let the Nexus or a follow-up task disposition it.
